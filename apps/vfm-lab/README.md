...
Phase 1 could prove — and each now has a Phase 2 spec that proves the real
behaviour.
...
- **Scroll lock** is observable, but the numbers are not realistic:
  ...
  equals the full `window.innerWidth`. **Bridged by `e2e/scroll.e2e.ts`**: a
  genuinely scrollable page, wheel input, and a pixel-exact gap compensation
  check against a real scrollbar.
